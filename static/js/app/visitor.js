...
                }
		...
		addOfflineLabel = function() {
			...
			var offlineFormName = "Contact Us";
			if(typeof(offlineForm) != 'undefined') {
				offlineFormName = offlineForm.name;
			}
			console.log('Adding offline label');
			$('#bakbakchat').empty();
			$('#offlineConfirm').unbind('click');
			$('#bakbakchat').removeClass('extraImage');
			$('#bakbakchat').append("<div id='offlineConfirm' class='alert alert-danger' style='margin:0px'><span class='chatHeaderText'>"+ offlineFormName +"</span><img src='"+bakbakUrl+"img/avatars/avatar-red-talking20x20.png'></img></div>");
			$('#bakbakchat').removeClass('chatMaximize').addClass('chatMinimize');
